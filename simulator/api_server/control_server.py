...
from irobot_create_msgs.action import (
    Undock as UndockAction,
)  # Added for Undock action type
...
class RobotControllerNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            "ROS2 node for Flask robot controller initialized and /cmd_vel (TwistStamped) publisher created."
        )
...
def init_ros2_node():
    ...
    ros_spin_thread = threading.Thread(
        target=lambda: rclpy.spin(ros2_node), daemon=True
    )
...
def control_robot():
    ...
    if cmd_vel_publisher is None or ros2_node is None:
        return (
            jsonify(
                {
                    "status": "error",
                    "message": "ROS2 publisher or node not initialized.",
                }
            ),
            503,
        )
    ...
    try:
        ...
        ros2_node.get_logger().info(
            f"Published to /cmd_vel (TwistStamped): linear.x={linear_x}, angular.z={angular_z}, stamp={twist_stamped_msg.header.stamp.sec}.{twist_stamped_msg.header.stamp.nanosec}"
        )
        ...
        if ros2_node:
            ros2_node.get_logger().error(
                f"Type error processing request: {e}. Data received: {request.data}"
            )
        return (
            jsonify({"status": "error", "message": f"Invalid data type in JSON: {e}"}),
            400,
        )
    except ValueError as e:
        if ros2_node:
            ros2_node.get_logger().error(
                f"Value error processing request: {e}. Data received: {data}"
            )
        return (
            jsonify({"status": "error", "message": f"Invalid value in JSON: {e}"}),
            400,
        )
...
def stop_robot():
    ...
    try:
        ...
        cmd_vel_publisher.publish(twist_stamped_msg)
        ros2_node.get_logger().info(
            f"Published stop command to /cmd_vel (TwistStamped), stamp={twist_stamped_msg.header.stamp.sec}.{twist_stamped_msg.header.stamp.nanosec}"
        )
        return (
            jsonify({"status": "success", "message": "Stop command sent to robot."}),
            200,
        )
    except Exception as e:
        if ros2_node:
...
def undock_robot():
    ...
    if ros2_node is None or undock_action_client is None:
        return (
            jsonify(
                {
                    "status": "error",
                    "message": "ROS2 node or undock action client not initialized.",
                }
            ),
            503,
        )
    ...
    if not undock_action_client.wait_for_server(timeout_sec=server_wait_timeout):
        msg = (
            f"/undock action server not available after waiting {server_wait_timeout}s."
        )
    ...
    try:
        ...
        while not goal_handle_future.done():
            if time.time() - start_time > goal_acceptance_timeout_sec:
                ros2_node.get_logger().error(
                    f"Timeout waiting for undock goal acceptance after {goal_acceptance_timeout_sec}s."
                )
                return (
                    jsonify(
                        {
                            "status": "error",
                            "message": "Timeout waiting for undock goal acceptance.",
                        }
                    ),
                    504,
                )
            time.sleep(
                0.01
            )  # Yield control to allow other threads (like rclpy spin) to run
        ...
        ros2_node.get_logger().info(
            "Undock goal accepted by server. Waiting for result..."
        )
        ...
        while not result_future.done():
            if time.time() - start_time_result > action_completion_timeout_sec:
                ros2_node.get_logger().error(
                    f"Timeout waiting for undock action result after {action_completion_timeout_sec}s."
                )
                ...
                return (
                    jsonify(
                        {
                            "status": "error",
                            "message": "Timeout waiting for undock action result.",
                        }
                    ),
                    504,
                )
        ...
        )  # This object contains 'status' and 'result' (the Undock.Result message)
        ...
        if action_response.status == rclpy.action.GoalStatus.STATUS_SUCCEEDED:
            ...
            return (
                jsonify({"status": "success", "message": "Undock action succeeded."}),
                200,
            )
        else:
            # Log other statuses like ABORTED, CANCELED, etc.
            status_name = rclpy.action.GoalStatus.to_string(
                action_response.status
            )  # Helper to get string name
            ros2_node.get_logger().info(
                f"Undock action failed with status: {status_name} ({action_response.status})"
            )
            return (
                jsonify(
                    {
                        "status": "error",
                        "message": f"Undock action failed with status: {status_name}",
                    }
                ),
                500,
            )
...
if __name__ == "__main__":
    try:
        ...
        if not initialization_ok:
            ...
            print(
                '  POST /control_robot  (JSON: {"linear_x": float, "angular_z": float})'
            )
